...
class ForecastViewModel extends ChangeNotifier {
  ...
  Future<void> fetchForecast({String? cityName, int days = 1}) async {
    if (cityName == null) {
      ...
      _weatherData = await getForecastForLocation.getForecastByLocation(location: position, days: days);
    } else {
      _weatherData = await getForecastForLocation.getForecastByLocation(city: cityName, days: days);
    }
    notifyListeners();
  }

  Future<void> fetchForecastByDay({required String cityName, required DateTime date}) async {
    _weatherData = await getForecastByCityAndDate.getForecastByCityAndDate(city: cityName, date: date);
    notifyListeners();
  }
}
